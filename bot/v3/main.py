...
def findImage(id):
    ...
    if id.upper() == id.lower():
        parsedId = bot_Util.parseThis(id)
    # When Eng
...
async def on_message(message):
    ...
    if topic is not None and f'#{bot.user.name}' in topic:
        # [명령어]
        ...
        if message.content.startswith('!링크'):
            ...
            print(f'\n[명령어]: {messageByWord[0]}')
            print(f'[author.nick]: {message.author.nick}(name: {message.author.name})')
            await channel.send(Talents.getLink(messageByWord[1]))
            ...
            if messageByWord[1] in doubleNamed:
                ...
                try:
                    ...
                    if messageByWord[0] == '!특성':
                        ...
                        #     if word in value:
                        #         className = key
                        await channel.send('2-3분 정도 기다려 주세요.\n끝나면 알려드릴께요. (추가 명령 금지:face_with_peeking_eye:)')
                        await channel.send(f'**{word}**에 대한 :thumbsup:**BIS**에요.\n{Talents.getBIS(word)}')
                        await channel.send(f'\n:ballot_box_with_check:{message.author.mention}님! 완료되었습니다!')
                        await channel.send(
                            f':arrow_forward: 자세한 건 와우헤드로 가보세요.\n:arrow_forward: !링크 {messageByWord[1]}라고 치면 링크를 알려드릴게요.')
                ...
                if messageByWord[0] == '!특성':
                    for key, value in Talents.byClass.items():
                        if messageByWord[1] in value:
                            className = key
                    await channel.send(f'{messageByWord[1]} {classNameDic[className]}에 대한 특성이에요.\n\n{Talents.getTalent(messageByWord[1])}')
                #     message.author.dm_channel.send
                    ...
                    for key, value in Talents.byClass.items():
                        if messageByWord[1] in value:
                            className = key
                    await channel.send('2-3분 정도 기다려 주세요.\n끝나면 알려드릴께요. (추가 명령 금지:face_with_peeking_eye:)')
                    await channel.send(f'**{messageByWord[1]} {classNameDic[className]}**에 대한 :thumbsup:**BIS**에요.\n{Talents.getBIS(messageByWord[1])}')
                    await channel.send(f'\n{message.author.mention}님! :ballot_box_with_check:완료되었습니다!')
                    await channel.send(f':arrow_forward: 자세한 건 와우헤드로 가보세요.\n:arrow_forward: !링크 {messageByWord[1]}라고 치면 링크를 알려드릴게요.')
        ...
        if message.content.startswith('!누구'):
            ...
            characterName = bot_Util.parseThis(messageByWord[1])
            characterInfo = Character.getCharacterInfo(characterName, access_token)
            characterInfo = characterInfo.split(',')
            await channel.send(f'{messageByWord[1]}님은 {characterInfo[0]}..\n{characterInfo[1]} {characterInfo[2]}에요.\n아이템 레벨은 {characterInfo[3]}정도 됐어요.')
